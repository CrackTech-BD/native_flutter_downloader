...
public class SwiftNativeFlutterDownloaderPlugin: NSObject, FlutterPlugin {
    private static let kFlutterChannelName: String = "org.cracktech.native_flutter_downloader"
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = SwiftNativeFlutterDownloaderPlugin()
        registrar.addMethodCallDelegate(instance, channel: channel!)
    }

    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        let arguments: Dictionary<String, Any> = call.arguments as! Dictionary<String, Any>
        if call.method == SwiftNativeFlutterDownloaderPlugin.kDownloadMethodName {
            ...
            result(taskId)
        } else if call.method == SwiftNativeFlutterDownloaderPlugin.kOpenFileMethodName {
            openFile(path: arguments["filePath"] as! String)
        } else if call.method == SwiftNativeFlutterDownloaderPlugin.kCancelMethodName {
    ...
    private func download(url: String, headers: [String: String]?, fileName: String?) -> Int {
        ...
        if prefs.object(forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD) != nil {
            var downloadNames = prefs.array(forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD)
            let dict = ["url": downloadTask.originalRequest?.url?.absoluteString ?? "",
                        "fileName": fileName ?? ""] as [String : Any]
            downloadNames?.append(dict)
            prefs.set(downloadNames, forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD)
        } else {
            let dict = ["url": downloadTask.originalRequest?.url?.absoluteString ?? "",
                        "fileName": fileName ?? ""] as [String : Any]
            let list: Array = [dict]
            prefs.set(list, forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD)
        }
...
extension SwiftNativeFlutterDownloaderPlugin: URLSessionDelegate, URLSessionDownloadDelegate {
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo fileURL: URL) {
        var downloadNames = UserDefaults.standard.array(forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD)
        var fileName: String?
        ...
        do {
            ...
            if let dict = downloadNames?.first(where: {
                ...
                UserDefaults.standard.set(downloadNames, forKey: SwiftNativeFlutterDownloaderPlugin.kDownloadNamesUD)
            }
            ...
            do {
                ...
                SwiftNativeFlutterDownloaderPlugin.channel?.invokeMethod(SwiftNativeFlutterDownloaderPlugin.kNotifyProgressMethodName, arguments:[
                ...
                if #available(iOS 14.0, *) {
                    let logger = Logger(subsystem: Bundle.main.bundleIdentifier!, category: "native_flutter_downloader")
...
extension SwiftNativeFlutterDownloaderPlugin: UIDocumentInteractionControllerDelegate {
